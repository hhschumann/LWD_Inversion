...
When considering the drilling industry, the field of geosteering is moving in the direction of automated guidance. For our work, we use a synthetic pilot well gamma ray log and vertical depth measuremnts and sythetic wellbore coordinates and LWD gamma ray measurements to invert for the distance of the well bore from the top of a particular geologic interval of interest. Gamma ray logs have been used previously in fast forward algorithms to obtain similar results in real time [4]. However, in this report we build a linearized inversion algorithm using Tikhonov regularization method to solve this nonlinear problem.

***Data***

For our example, we created a synthetic LWD gamma ray log and wellbore subsurface coordinates for a fake horizontal unconventional well in addition to a synthetic gamma ray log and vertical depth measuremnts for a fake vertical pilot well. We also  This artificial dataset was used to evaluate the method's effectiveness since the true model can be known. Additionally, read datasets of this type are typically not available to the public.  
